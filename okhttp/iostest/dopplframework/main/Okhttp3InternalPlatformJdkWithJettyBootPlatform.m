...
- (NSString *)getSelectedProtocolWithJavaxNetSslSSLSocket:(JavaxNetSslSSLSocket *)socket {
  @try {
    ...
    if (!((Okhttp3InternalPlatformJdkWithJettyBootPlatform_JettyNegoProvider *) nil_chk(provider))->unsupported_ && provider->selected_ == nil) {
      [((Okhttp3InternalPlatformPlatform *) nil_chk(Okhttp3InternalPlatformPlatform_get())) logWithInt:Okhttp3InternalPlatformPlatform_INFO withNSString:@"ALPN callback dropped: SPDY and HTTP/2 are disabled. Is alpn-boot on the boot class path?" withNSException:nil];
      return nil;
    }
